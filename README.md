...
### Secondary use-cases
It is always running so sensors can push data to it (via HTTP GET) and prometheus can pull from it. It also pushes data to MQTT broker on behalf of the sensors so the sensors can have only single upload method (again, saving run time and thus battery).
